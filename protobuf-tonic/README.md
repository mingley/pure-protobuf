...
dropping the per-message `Vec`. Remaining gap is `Parse`. Parse-only
hello (no codec framing) is 44.7 vs 22.1. Leftover is a discarded
parent-frame `Arc`, not UTF-8. Still a loss. Not a win. Not kernel
`./bench`.
