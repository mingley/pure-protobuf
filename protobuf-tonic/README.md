...
dropping the per-message `Vec`. Remaining gap is `Parse`. Leftover after
the #34 Arc skip is `merge_inner` glue (inventory running); #32 stays
draft. Not a win. Not kernel `./bench`.
