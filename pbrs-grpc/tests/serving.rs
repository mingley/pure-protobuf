...
fn channel_call_apis_document_hand_written_services() {
    ...
    assert!(
        intercept.contains("A non-OK peer status skips this hook."),
        "ResponseInterceptor rustdoc must name client skip on non-OK peer status"
    );
    assert!(
        intercept.contains("the peer already sent OK"),
    ...
    assert!(
        src.contains("A non-OK peer status skips"),
        "Channel::on_response must name skip on non-OK peer status"
    );
    assert!(
...
//! server."
        ...
        "testing crate rustdoc must name send-buffer still-serves on every TestService shape"
    );
    assert!(
        testing.contains(
...
//! server."
        ...
        "testing crate rustdoc must name HTTP/2 window still-serves on every TestService shape"
    );
    assert!(
        testing.contains(
...
fn request_deadline_documents_every_transport() {
    ...
    assert!(
        src.contains("A [`crate::Channel::on_response`] hook can insert after a"),
        "Response::extensions must name Channel::on_response as the receive-side insert"
    );
    assert!(
        src.contains(
...
async fn echo_client_hook_every_shape(client: &GreeterClient) {
    let resp = client
        .say_hello(Request::new(req("ada")))
        .await
        .expect("unary");
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn channel_on_response_inserts_local_extension_after_receive() {
    let (addr, listener) = bind().await;
    let task = tokio::spawn(async move {
        GreeterServer::new(Echo)
            .on_response(stamp_hook)
    ...
    let client = GreeterClient::new(channel(addr).await).on_response(insert_local_ext);
    let reply = client
        .say_hello(Request::new(req("ada")))
        .await
        .expect("unary");
    ...
    assert_eq!(reply.metadata().get("x-hook"), Some("1"));
    assert_eq!(
        reply.extensions().get::<u8>().copied(),
        Some(7),
        "client on_response must insert local extensions the peer cannot"
    );
    task.abort();
}

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn channel_on_response_every_greeter_shape() {
    let (addr, listener) = bind().await;
    let task = tokio::spawn(async move {
        GreeterServer::new(Echo)
            .on_response(stamp_hook)
    ...
    echo_client_hook_every_shape(
        &GreeterClient::new(channel(addr).await).on_response(insert_local_ext),
    )
    .await;
    task.abort();
}
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn channel_on_response_err_after_peer_ok() {
    let ran = Arc::new(AtomicUsize::new(0));
    let (addr, listener) = bind().await;
    let task = tokio::spawn({
        let ran = Arc::clone(&ran);
    ...
    let client = GreeterClient::new(channel(addr).await).on_response(deny_after_ok);
    let err = client
        .say_hello(Request::new(req("ada")))
        .await
...
async fn channel_on_response_skips_non_ok() {
    ...
    let client = GreeterClient::new(channel(addr).await).on_response({
        let ran = Arc::clone(&ran);
        move |_: &mut ResponseParts| {
            ran.fetch_add(1, Ordering::SeqCst);
            Ok(())
        }
    });
    ...
    assert_eq!(
        ran.load(Ordering::SeqCst),
        0,
        "non-OK peer status must skip the client response interceptor"
    );
    task.abort();
}

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn channel_on_response_stack_is_first_registered_first() {
    let (addr, listener) = bind().await;
    let task = tokio::spawn(async move {
        GreeterServer::new(Echo).serve_listener(listener).await.ok();
    });
...
fn test_send_buffer() -> TestServiceServer<InteropTestService> {
    TestServiceServer::new(InteropTestService).max_send_buffer_size(16 * 1024)
}

#[tokio::test]
async fn test_service_send_buffer_still_serves_every_shape() {
    let (addr, listener) = bind().await;
    let task = tokio::spawn(async move {
        test_send_buffer().serve_listener(listener).await.ok();
    });
    echo_test_every_shape(&TestServiceClient::new(channel(addr).await)).await;
    task.abort();
}
...
#[tokio::test]
async fn tls_test_service_send_buffer_still_serves_every_shape() {
    let tls = ServerTls::new(server_identity()).expect("server tls");
    let (addr, listener) = bind().await;
    let task = tokio::spawn(async move {
        test_send_buffer()
...
#[tokio::test]
async fn mtls_test_service_send_buffer_still_serves_every_shape() {
    let tls = ServerTls::mtls(server_identity(), CA).expect("mtls server");
    let (addr, listener) = bind().await;
    let task = tokio::spawn(async move {
        test_send_buffer()
...
#[cfg(unix)]
#[tokio::test]
async fn unix_test_service_send_buffer_still_serves_every_shape() {
    let (path, _guard) = unix_test_path();
    let sock = path.clone();
    let task = tokio::spawn(async move {
        test_send_buffer().serve_unix(sock).await.ok();
    });
    echo_test_every_shape(&TestServiceClient::new(unix_channel(&path).await)).await;
    task.abort();
}
...
#[tokio::test]
async fn from_io_test_service_send_buffer_still_serves_every_shape() {
    let (client_io, server_io) = duplex_pair();
    let server = tokio::spawn(async move {
        test_send_buffer().serve_connection(server_io).await.ok();
    });
    echo_test_every_shape(&TestServiceClient::new(
        Channel::from_io(client_io, "localhost")
            .await
...
fn reverser_send_buffer() -> Server<Reverser> {
    Server::new(Reverser::new(Arc::new(AtomicUsize::new(0)))).max_send_buffer_size(16 * 1024)
}

fn reverser_mtls_send_buffer() -> Server<Reverser> {
    Server::new(Reverser::mtls(
        Arc::new(AtomicUsize::new(0)),
        client_identity().certificates().next().expect("leaf"),
    ))
...
#[tokio::test]
async fn reverser_send_buffer_still_serves_every_shape() {
    let (addr, listener) = bind().await;
    let task = tokio::spawn(async move {
        reverser_send_buffer().serve_listener(listener).await.ok();
    });
    echo_reverser_every_shape(&channel(addr).await).await;
    task.abort();
}
...
#[tokio::test]
async fn tls_reverser_send_buffer_still_serves_every_shape() {
    let tls = ServerTls::new(server_identity()).expect("server tls");
    let (addr, listener) = bind().await;
    let task = tokio::spawn(async move {
        reverser_send_buffer()
            .serve_tls_with_shutdown(listener, std::future::pending(), tls)
            .await
            .ok();
    });
...
#[tokio::test]
async fn mtls_reverser_send_buffer_still_serves_every_shape() {
    let tls = ServerTls::mtls(server_identity(), CA).expect("mtls server");
    let (addr, listener) = bind().await;
    let task = tokio::spawn(async move {
        reverser_mtls_send_buffer()
            .serve_tls_with_shutdown(listener, std::future::pending(), tls)
            .await
            .ok();
    });
...
#[cfg(unix)]
#[tokio::test]
async fn unix_reverser_send_buffer_still_serves_every_shape() {
    let (path, _guard) = unix_test_path();
    let sock = path.clone();
    let task = tokio::spawn(async move {
        reverser_send_buffer().serve_unix(sock).await.ok();
    });
    echo_reverser_every_shape(&unix_channel(&path).await).await;
    task.abort();
}
...
#[tokio::test]
async fn from_io_reverser_send_buffer_still_serves_every_shape() {
    let (client_io, server_io) = duplex_pair();
    let server = tokio::spawn(async move {
        reverser_send_buffer()
            .serve_connection(server_io)
...
fn test_window_size() -> TestServiceServer<InteropTestService> {
    TestServiceServer::new(InteropTestService)
        .initial_stream_window_size(64 * 1024)
        .initial_connection_window_size(128 * 1024)
}
...
#[tokio::test]
async fn test_service_window_size_still_serves_every_shape() {
    let (addr, listener) = bind().await;
    let task = tokio::spawn(async move {
        test_window_size().serve_listener(listener).await.ok();
    });
    echo_test_every_shape(&TestServiceClient::new(channel(addr).await)).await;
    task.abort();
}
...
#[tokio::test]
async fn tls_test_service_window_size_still_serves_every_shape() {
    let tls = ServerTls::new(server_identity()).expect("server tls");
    let (addr, listener) = bind().await;
    let task = tokio::spawn(async move {
        test_window_size()
...
#[tokio::test]
async fn mtls_test_service_window_size_still_serves_every_shape() {
    let tls = ServerTls::mtls(server_identity(), CA).expect("mtls server");
    let (addr, listener) = bind().await;
    let task = tokio::spawn(async move {
        test_window_size()
...
#[cfg(unix)]
#[tokio::test]
async fn unix_test_service_window_size_still_serves_every_shape() {
    let (path, _guard) = unix_test_path();
    let sock = path.clone();
    let task = tokio::spawn(async move {
        test_window_size().serve_unix(sock).await.ok();
    });
    echo_test_every_shape(&TestServiceClient::new(unix_channel(&path).await)).await;
    task.abort();
}
...
#[tokio::test]
async fn from_io_test_service_window_size_still_serves_every_shape() {
    let (client_io, server_io) = duplex_pair();
    let server = tokio::spawn(async move {
        test_window_size().serve_connection(server_io).await.ok();
    });
    echo_test_every_shape(&TestServiceClient::new(
        Channel::from_io(client_io, "localhost")
            .await
...
fn reverser_window_size() -> Server<Reverser> {
    Server::new(Reverser::new(Arc::new(AtomicUsize::new(0))))
        .initial_stream_window_size(64 * 1024)
        .initial_connection_window_size(128 * 1024)
}

fn reverser_mtls_window_size() -> Server<Reverser> {
    Server::new(Reverser::mtls(
        Arc::new(AtomicUsize::new(0)),
        client_identity().certificates().next().expect("leaf"),
    ))
...
#[tokio::test]
async fn reverser_window_size_still_serves_every_shape() {
    let (addr, listener) = bind().await;
    let task = tokio::spawn(async move {
        reverser_window_size().serve_listener(listener).await.ok();
    });
    echo_reverser_every_shape(&channel(addr).await).await;
    task.abort();
}
...
#[tokio::test]
async fn tls_reverser_window_size_still_serves_every_shape() {
    let tls = ServerTls::new(server_identity()).expect("server tls");
    let (addr, listener) = bind().await;
    let task = tokio::spawn(async move {
        reverser_window_size()
            .serve_tls_with_shutdown(listener, std::future::pending(), tls)
            .await
            .ok();
    });
...
#[tokio::test]
async fn mtls_reverser_window_size_still_serves_every_shape() {
    let tls = ServerTls::mtls(server_identity(), CA).expect("mtls server");
    let (addr, listener) = bind().await;
    let task = tokio::spawn(async move {
        reverser_mtls_window_size()
            .serve_tls_with_shutdown(listener, std::future::pending(), tls)
            .await
            .ok();
    });
...
#[cfg(unix)]
#[tokio::test]
async fn unix_reverser_window_size_still_serves_every_shape() {
    let (path, _guard) = unix_test_path();
    let sock = path.clone();
    let task = tokio::spawn(async move {
        reverser_window_size().serve_unix(sock).await.ok();
    });
    echo_reverser_every_shape(&unix_channel(&path).await).await;
    task.abort();
}
...
#[tokio::test]
async fn from_io_reverser_window_size_still_serves_every_shape() {
    let (client_io, server_io) = duplex_pair();
    let server = tokio::spawn(async move {
        reverser_window_size()
            .serve_connection(server_io)
