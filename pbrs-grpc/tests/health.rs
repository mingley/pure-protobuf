...
fn health_crate_docs_name_interceptor_wait_for_ready() {
    ...
    assert!(
        ...
        "Health crate rustdoc must Distinct server on_response Err trailers-only after handler Ok from this health interceptor Err without reading the body"
    );
    assert!(
        src.contains(
        ...
        "Health crate rustdoc must Distinct client interceptor Err local reject never opens a stream from this health interceptor Err without reading the body"
    );
    assert!(
        src.contains(
    ...
    assert!(
        ...
        "Health crate rustdoc must Distinct interceptor Err without reading the body from this health handler Err after the handler ran"
    );
    assert!(
        src.contains(
        ...
        "Health crate rustdoc must Distinct client interceptor Err local reject never opens a stream from this health handler Err after the handler ran"
    );
    assert!(
        src.contains(
        ...
        "Health crate rustdoc must Distinct server on_response Err trailers-only after handler Ok from this health handler Err after the handler ran"
    );
    assert!(
        src.contains(
        ...
        "Health crate rustdoc must Distinct client on_response Err Call fail after receive from this health handler Err after the handler ran"
    );
    assert!(
        src.contains(
        ...
        "Health crate rustdoc must Distinct StreamSender fail after any messages already sent from this health handler Err after the handler ran"
    );
    assert!(
        src.contains(
    ...
    assert!(
        ...
        "Health crate rustdoc must Distinct health handler Err after the handler ran from this health client interceptor Err local reject never opens a stream"
    );
    assert!(
        src.contains(
        ...
        "Health crate rustdoc must Distinct client on_response Err Call fail after receive from this health client interceptor Err local reject never opens a stream"
    );
    assert!(
        src.contains(
        ...
        "Health crate rustdoc must Distinct interceptor Err without reading the body from this health client interceptor Err local reject never opens a stream"
    );
    assert!(
        src.contains(
    ...
    assert!(
        ...
        "Health crate rustdoc must Distinct handler Err after the handler ran from this health StreamSender fail after any messages already sent"
    );
    assert!(
        src.contains(
        ...
        "Health crate rustdoc must Distinct interceptor Err without reading the body from this health StreamSender fail after any messages already sent"
    );
    assert!(
        src.contains(
        ...
        "Health crate rustdoc must Distinct server on_response Err trailers-only after handler Ok from this health StreamSender fail after any messages already sent"
    );
    assert!(
        src.contains(
        ...
        "Health crate rustdoc must Distinct client interceptor Err local reject never opens a stream from this health StreamSender fail after any messages already sent"
    );
    assert!(
        src.contains(
        ...
        "Health crate rustdoc must Distinct client on_response Err Call fail after receive from this health StreamSender fail after any messages already sent"
    );
    assert!(
        src.contains(
        ...
        "Health crate rustdoc must name from_error_details typed bag next to health server on_response Err"
    );
    assert!(
        src.contains(
    ...
    assert!(
        ...
        "Health crate rustdoc must Distinct interceptor Err without reading the body from this health server on_response Err trailers-only after handler Ok"
    );
    assert!(
        src.contains(
            "Distinct from a health client on_response Err: that fails the Call after a successful receive; this health server on_response Err is trailers-only after handler Ok."
        ),
        "Health crate rustdoc must Distinct health client on_response Err Call fail after receive from this health server on_response Err trailers-only after handler Ok"
    );
    assert!(
        src.contains(
    ...
    assert!(
        src.contains(
            "Distinct from a health interceptor Err: that is trailers without reading the body; this health client on_response Err fails the Call after a successful receive."
        ),
        "Health crate rustdoc must Distinct interceptor Err without reading the body from this health client on_response Err Call fail after receive"
    );
    assert!(
        src.contains(
        ...
        "Health crate rustdoc must Distinct client interceptor Err local reject never opens a stream from this health client on_response Err Call fail after receive"
    );
    assert!(
        src.contains(
            "Distinct from a health server on_response Err: that is trailers-only after handler Ok; this health client on_response Err fails the Call after a successful receive."
        ),
        "Health crate rustdoc must Distinct health server on_response Err trailers-only after handler Ok from this health client on_response Err Call fail after receive"
    );
    assert!(
        src.contains(
            "Distinct from a health StreamSender fail: that is trailers after any messages already sent; this health client on_response Err fails the Call after a successful receive."
        ),
        "Health crate rustdoc must Distinct StreamSender fail after any messages already sent from this health client on_response Err Call fail after receive"
    );
}
