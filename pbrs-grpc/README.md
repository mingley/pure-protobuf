...
`Code::is_retryable` / `Status::is_retryable` (gRPC A6: `UNAVAILABLE` only), `Status::error_info` / `ErrorInfo::with_reason` / `ErrorInfo::with_metadata`, `Status::retry_delay` / `RetryInfo::with_retry_delay`, `Status::from_error` wrapping local errors,
