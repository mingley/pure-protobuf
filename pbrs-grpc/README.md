...
(server `Rpc`/`Request` metadata/timeout/deadline/`peer_timeout`/`:authority`/`:scheme`/path/service/method/`local_addr`/`peer_identity`/`peer_cred`/`limits`/`accepts_gzip`/`encoding`/extensions, client `Outgoing` with path/service/method, `:authority`, `:scheme`, `user-agent`, message caps, deadline, wait-for-ready, compression, caller and stacked-interceptor extensions),
...
cancellation (dropping a `Call` resets the stream; `Request::cancelled` for spawned work), ASCII and `-bin` metadata, OK-path custom trailers,
