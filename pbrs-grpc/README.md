...
(server `Rpc`/`Request` metadata/timeout/deadline/`:authority`/`:scheme`/`local_addr`/`peer_identity`/`peer_cred`/`limits`/extensions, client `Outgoing` with path/service/method, `:authority`, `:scheme`, `user-agent`, message caps, deadline, wait-for-ready, compression, caller and stacked-interceptor extensions),
