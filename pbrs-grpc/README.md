...
(server `Rpc`/`Request` metadata/timeout/deadline/`:authority`/`:scheme`/`local_addr`/`peer_identity`/extensions, client `Outgoing` with path, `:authority`, deadline, wait-for-ready, compression, caller and stacked-interceptor extensions),
