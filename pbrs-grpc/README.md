...
`Code::is_retryable` / `Status::is_retryable` (gRPC A6: `UNAVAILABLE` only), `Status::error_info` / `ErrorInfo::with_reason`, `Status::retry_delay` from packed `RetryInfo`, `Status::from_error` wrapping local errors,
