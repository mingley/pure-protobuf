...
cancellation (dropping a `Call` or a received `Streaming` resets the stream; a `CallHandle` taken before await still cancels after streaming headers and after a client-streaming sender is closed; `Request::cancelled` for spawned work), ASCII and `-bin` metadata, OK-path custom trailers,
