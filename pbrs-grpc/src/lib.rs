...
//!
//! There is no tonic `ServerTlsConfig::client_auth_optional`: that requests a client certificate but does not require one. This crate-map [`ServerTls::mtls`] always requires a client certificate issued by that CA. Distinct from [`ServerTls::new`] (clients are not asked). Distinct from a skip-verify constructor (there is none). Distinct from [`ClientTls::ca_mtls`] / [`ClientTls::webpki_mtls`] (client presents; this is the server require).
//!
