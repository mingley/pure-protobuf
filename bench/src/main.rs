//! Same-schema encode/decode vs prost, protobuf v4 (upb), and buffa.

use buffa::{Message as BuffaMessage, MessageView};
use buffa_tat::protobuf_test_messages::proto3::{
    test_all_types_proto3::NestedMessage as BuffaNested, TestAllTypesProto3 as BuffaTat,
    TestAllTypesProto3View as BuffaTatView,
};
use pbrs::gencode::{NestedMessage, TestAllTypesProto3};
use pbrs::prelude::*;
use pbrs::testdata::{Address, Person};
use prost::Message;
use protobuf_v4::{Parse as V4Parse, Serialize as V4Serialize};
use std::time::Instant;


/// Generated Person schema layout from `proto/person.proto` via protoc-gen-pbrs.
/// Used to isolate the layout difference between handwritten and compiler-generated messages.
#[allow(
    unused,
    non_snake_case,
    non_camel_case_types,
    clippy::all,
    unreachable_pub,
    reason = "generated by protoc-gen-pbrs"
)]
pub mod person_generated {
// @generated by protoc-gen-pbrs
#[allow(unused, non_snake_case, non_camel_case_types, clippy::all, clippy::restriction, clippy::indexing_slicing, clippy::cast_possible_truncation, clippy::cast_possible_wrap, clippy::cast_sign_loss, clippy::let_underscore_must_use, unreachable_pub, reason = "generated by protoc-gen-pbrs")]
mod __gen {
#![allow(unused, non_snake_case, non_camel_case_types, clippy::all, clippy::restriction, clippy::indexing_slicing, clippy::cast_possible_truncation, clippy::cast_possible_wrap, clippy::cast_sign_loss, clippy::let_underscore_must_use, unsafe_code, unreachable_pub, reason = "generated by protoc-gen-pbrs")]
use pbrs::prelude::*;
use pbrs::{Enum, Map, MapMut, MapView, ParseError, ProtoBytes, ProtoString, Repeated, RepeatedMut, RepeatedView, SerializeError, UnknownEnumValue};
use pbrs::UnknownFields;

pub const FILE_DESCRIPTOR_SET: &[u8] = &[
    ,
];

fn generated_pool() -> std::sync::Arc<pbrs::DescriptorPool> {
    static P: std::sync::OnceLock<std::sync::Arc<pbrs::DescriptorPool>> = std::sync::OnceLock::new();
    P.get_or_init(|| {
        std::sync::Arc::new(pbrs::DescriptorPool::from_file_descriptor_set(FILE_DESCRIPTOR_SET).expect("fds"))
    }).clone()
}

#[derive(Clone, Debug)]
pub struct Address {
    city: pbrs::rt::LazyStr,
    unknown: UnknownFields,
    cached_size: pbrs::rt::CachedSize,
}
impl PartialEq for Address {
    fn eq(&self, other: &Self) -> bool {
        if self.city != other.city { return false; }
        self.unknown == other.unknown
    }
}
impl Eq for Address {}
impl Default for Address {
    #[inline(always)]
    fn default() -> Self {
        unsafe { pbrs::rt::zeroed_message() }
    }
}
impl Address {
    pub fn new() -> Self { Self::default() }
    pub const EMPTY_PARSE_OK: bool = true;
    pub const FULL_NAME: &'static str = "example.Address";
    pub fn city(&self) -> &pbrs::ProtoStr { Some(self.city.as_view()).unwrap_or_else(|| pbrs::ProtoStr::from_bytes(b"")) }
    pub fn set_city(&mut self, v: impl pbrs::IntoProxied<ProtoString>) { self.cached_size.dirty(); self.city = pbrs::rt::LazyStr::owned(v.into_proxied(pbrs::__internal::Private)); }
    #[inline(always)] fn check_required(&self) -> Result<(), ParseError> {
        Ok(())
    }
    fn merge_bytes(&mut self, data: &[u8], depth: u32) -> Result<(), ParseError> { if data.is_empty() { return self.check_required(); } let mut pos = 0; let mut wire = None; self.merge_inner(data, &mut wire, &mut pos, depth, true, None) }
    fn merge_bytes_dont_enforce(&mut self, data: &[u8], depth: u32) -> Result<(), ParseError> { if data.is_empty() { return Ok(()); } let mut pos = 0; let mut wire = None; self.merge_inner(data, &mut wire, &mut pos, depth, false, None) }
    fn merge_group(&mut self, data: &[u8], wire: &mut Option<pbrs::rt::Wire>, pos: &mut usize, num: u32, depth: u32) -> Result<(), ParseError> { self.merge_inner(data, wire, pos, depth, false, Some(num)) }
    fn merge_inner(&mut self, data: &[u8], wire: &mut Option<pbrs::rt::Wire>, pos: &mut usize, depth: u32, enforce: bool, until: Option<u32>) -> Result<(), ParseError> {
        if depth > pbrs::RECURSION_LIMIT { return Err(ParseError::new("recursion limit exceeded")); } let _ = enforce;
        self.cached_size.dirty();
        while *pos < data.len() {
            let (n, w) = pbrs::rt::decode_tag(data, pos)?;
            if let Some(g) = until { if w == pbrs::rt::WIRE_EGROUP { if n != g { return Err(ParseError::new("mismatched end-group")); } return Ok(()); } }
            match n {
            1 => match w {
                pbrs::rt::WIRE_LEN => {
                    let (s, e) = pbrs::rt::read_len_span(data, pos)?; self.city = pbrs::rt::LazyStr::from_parse_span(wire, data, s, e)?;
                    }
                _ => self.unknown.fields.push(pbrs::rt::capture_unknown(data, pos, n, w)?),
            }
                _ => self.unknown.fields.push(pbrs::rt::capture_unknown(data, pos, n, w)?),
            }
        }
        if until.is_some() { return Err(ParseError::new("truncated group")); }
        if enforce { self.check_required()?; }
        Ok(())
    }
    fn validate_inner(wire: &pbrs::rt::Wire, pos: &mut usize, depth: u32) -> Result<(), ParseError> { Self::validate_until(wire, pos, depth, None) }
    fn validate_until(wire: &pbrs::rt::Wire, pos: &mut usize, depth: u32, until: Option<u32>) -> Result<(), ParseError> {
        if depth > pbrs::RECURSION_LIMIT { return Err(ParseError::new("recursion limit exceeded")); }
        let data = wire.as_slice();
        while *pos < data.len() {
            let (n, w) = pbrs::rt::decode_tag(data, pos)?;
            if let Some(g) = until { if w == pbrs::rt::WIRE_EGROUP { if n != g { return Err(ParseError::new("mismatched end-group")); } return Ok(()); } }
            match n {
            1 => match w {
                pbrs::rt::WIRE_LEN => { let (s, e) = pbrs::rt::read_len_span(data, pos)?; pbrs::rt::require_utf8(&data[s..e])?; }
                _ => pbrs::rt::skip_field(data, pos, w)?,
            }
                _ => pbrs::rt::skip_field(data, pos, w)?,
            }
        }
        if until.is_some() { return Err(ParseError::new("truncated group")); }
        Ok(())
    }
    fn compute_size(&self) -> u64 {
        if let Some(n) = self.cached_size.get() { return n; }
        let mut n = self.unknown.encoded_len();
        if !self.city.is_empty() { n += pbrs::rt::key_len_value_len(1, self.city.as_bytes().len() as u64); }
        self.cached_size.set(n);
        n
    }
    fn write_to(&self, out: &mut impl pbrs::rt::WireOut) {
        if !self.city.is_empty() { pbrs::rt::encode_len_field(out, 1, self.city.as_bytes()); }
        self.unknown.encode(out);
    }
    pub fn to_json(&self) -> Result<String, SerializeError> {
        Ok(self.to_json_value()?.to_string())
    }
    pub fn from_json(json: &str) -> Result<Self, ParseError> { Self::from_json_ignore(json, false) }
    pub fn from_json_ignore(json: &str, ignore: bool) -> Result<Self, ParseError> {
        let v = pbrs::json::parse(json)?;
        Self::from_json_value(&v, ignore)
    }
    fn to_json_value(&self) -> Result<pbrs::json::Json, SerializeError> {
        let mut map = pbrs::json::JsonMap::new();
        if !self.city().as_bytes().is_empty() {
            let _ = map.insert("city".into(), pbrs::json::string(self.city().as_bytes()));
        }
        Ok(pbrs::json::Json::Object(map))
    }
    fn from_json_value(v: &pbrs::json::Json, ignore: bool) -> Result<Self, ParseError> {
        let obj = v.as_object().ok_or_else(|| ParseError::new("json message must be an object"))?;
        let mut msg = Self::new();
        let mut seen = std::collections::BTreeSet::<u32>::new();
        for (key, val) in obj {
            match key.as_str() {
                "city" => {
                    if !seen.insert(1u32) { return Err(ParseError::owned(format!("duplicate json field {key}"))); }
                    if val.is_null() { continue; }
                    msg.set_city(pbrs::json::as_str(val)?);
                }
                _ => {
                    if !ignore { return Err(ParseError::owned(format!("unknown json field {key}"))); }
                }
            }
        }
        Ok(msg)
    }
    pub fn to_text(&self) -> Result<String, SerializeError> {
        let mut out = String::new();
        self.write_text(&mut out, 0)?;
        Ok(out)
    }
    pub fn to_text_with_unknown(&self) -> Result<String, SerializeError> {
        let mut out = String::new();
        self.write_text(&mut out, 0)?;
        pbrs::text::write_unknown_fields(&self.unknown, &mut out, 0);
        Ok(out)
    }
    pub fn from_text(text: &str) -> Result<Self, ParseError> {
        Self::from_text_value(&pbrs::text::parse(text)?)
    }
    fn write_text(&self, out: &mut String, indent: usize) -> Result<(), SerializeError> {
        if !self.city().as_bytes().is_empty() {
            pbrs::text::write_named_string(out, indent, "city", self.city().as_bytes());
        }
        Ok(())
    }
    fn from_text_value(fields: &[(String, pbrs::text::TextValue)]) -> Result<Self, ParseError> {
        let mut msg = Self::new();
        for (key, val) in fields {
            match key.as_str() {
                "city" => {
                    msg.set_city(val.as_str()?);
                }
                _ => return Err(ParseError::owned(format!("unknown field {key}"))),
            }
        }
        Ok(msg)
    }
}
pbrs::impl_typed_message!(Address, AddressView, AddressMut);
#[derive(Clone, Debug, PartialEq)]
struct PersonCold {
    name: pbrs::rt::LazyStr,
    email: Option<Box<pbrs::rt::LazyStr>>,
    tags: Repeated<pbrs::rt::LazyStr>,
    scores: Map<pbrs::rt::LazyStr, i32>,
    address: pbrs::rt::LazyMsg<Address>,
    extras: Map<pbrs::rt::LazyStr, i32>,
}
impl Default for PersonCold {
    #[inline(always)]
    fn default() -> Self { unsafe { pbrs::rt::zeroed_message() } }
}
#[derive(Clone, Debug)]
pub struct Person {
    id: i32,
    cold: Option<Box<PersonCold>>,
    unknown: UnknownFields,
    cached_size: pbrs::rt::CachedSize,
}
impl PartialEq for Person {
    fn eq(&self, other: &Self) -> bool {
        if self.id != other.id { return false; }
        match (self.cold.as_deref(), other.cold.as_deref()) {
            (None, None) => {}
            (Some(a), Some(b)) => if a != b { return false; }
            (None, Some(b)) => if *b != PersonCold::default() { return false; }
            (Some(a), None) => if *a != PersonCold::default() { return false; }
        }
        self.unknown == other.unknown
    }
}
impl Eq for Person {}
impl Default for Person {
    #[inline(always)]
    fn default() -> Self {
        unsafe { pbrs::rt::zeroed_message() }
    }
}
impl Person {
    pub fn new() -> Self { Self::default() }
    #[inline(always)] fn cold_mut(&mut self) -> &mut PersonCold { self.cold.get_or_insert_with(|| Box::new(PersonCold::default())) }
    pub const EMPTY_PARSE_OK: bool = true;
    pub const FULL_NAME: &'static str = "example.Person";
    pub fn id(&self) -> i32 { self.id }
    pub fn set_id(&mut self, v: i32) { self.cached_size.dirty(); self.id = v; }
    pub fn name(&self) -> &pbrs::ProtoStr { self.cold.as_ref().map(|c| c.name.as_view()).unwrap_or_else(|| pbrs::ProtoStr::from_bytes(b"")) }
    pub fn set_name(&mut self, v: impl pbrs::IntoProxied<ProtoString>) { self.cached_size.dirty(); self.cold_mut().name = pbrs::rt::LazyStr::owned(v.into_proxied(pbrs::__internal::Private)); }
    pub fn has_email(&self) -> bool { self.cold.as_ref().and_then(|c| c.email.as_ref()).is_some() }
    pub fn email(&self) -> &pbrs::ProtoStr { self.cold.as_ref().and_then(|c| c.email.as_ref()).map(|s| s.as_view()).unwrap_or_else(|| pbrs::ProtoStr::from_bytes(b"")) }
    pub fn email_opt(&self) -> Option<&pbrs::ProtoStr> { self.cold.as_ref().and_then(|c| c.email.as_ref()).map(|s| s.as_view()) }
    pub fn set_email(&mut self, v: impl pbrs::IntoProxied<ProtoString>) {
        self.cached_size.dirty();
        self.cold_mut().email = Some(Box::new(pbrs::rt::LazyStr::owned(v.into_proxied(pbrs::__internal::Private))));
    }
    pub fn clear_email(&mut self) { self.cached_size.dirty(); self.cold_mut().email = None; }
    pub fn tags(&self) -> RepeatedView<'_, pbrs::rt::LazyStr> { self.cold.as_ref().map(|c| c.tags.as_view()).unwrap_or_else(|| RepeatedView::from_slice(&[])) }
    pub fn tags_mut(&mut self) -> RepeatedMut<'_, pbrs::rt::LazyStr> { self.cached_size.dirty(); self.cold_mut().tags.as_mut() }
    pub fn set_tags(&mut self, v: impl IntoIterator<Item = pbrs::rt::LazyStr>) { self.cached_size.dirty(); self.cold_mut().tags = v.into_iter().collect(); }
    pub fn scores(&self) -> MapView<'_, pbrs::rt::LazyStr, i32> { self.cold.as_ref().map(|c| c.scores.as_view()).unwrap_or_else(MapView::empty) }
    pub fn scores_mut(&mut self) -> MapMut<'_, pbrs::rt::LazyStr, i32> { self.cached_size.dirty(); self.cold_mut().scores.as_mut() }
    pub fn set_scores(&mut self, v: Map<pbrs::rt::LazyStr, i32>) { self.cached_size.dirty(); self.cold_mut().scores = v; }
    pub fn has_address(&self) -> bool { self.cold.as_ref().is_some_and(|c| c.address.is_some()) }
    pub fn address(&self) -> &Address { self.cold.as_ref().and_then(|c| c.address.as_deref()).unwrap_or(pbrs::gen_support::default_instance_of()) }
    pub fn address_opt(&self) -> Option<&Address> { self.cold.as_ref().and_then(|c| c.address.as_deref()) }
    pub fn address_view(&self) -> AddressView<'_> { AddressView(self.address()) }
    pub fn set_address(&mut self, v: Address) {
        self.cached_size.dirty();
        self.cold_mut().address = pbrs::rt::LazyMsg::from_owned(v);
    }
    pub fn address_mut(&mut self) -> &mut Address { self.cached_size.dirty(); self.cold_mut().address.get_or_insert() }
    pub fn clear_address(&mut self) { self.cached_size.dirty(); self.cold_mut().address.clear(); }
    pub fn extras(&self) -> MapView<'_, pbrs::rt::LazyStr, i32> { self.cold.as_ref().map(|c| c.extras.as_view()).unwrap_or_else(MapView::empty) }
    pub fn extras_mut(&mut self) -> MapMut<'_, pbrs::rt::LazyStr, i32> { self.cached_size.dirty(); self.cold_mut().extras.as_mut() }
    pub fn set_extras(&mut self, v: Map<pbrs::rt::LazyStr, i32>) { self.cached_size.dirty(); self.cold_mut().extras = v; }
    #[inline(always)] fn check_required(&self) -> Result<(), ParseError> {
        Ok(())
    }
    fn merge_bytes(&mut self, data: &[u8], depth: u32) -> Result<(), ParseError> { if data.is_empty() { return self.check_required(); } let mut pos = 0; let mut wire = None; self.merge_inner(data, &mut wire, &mut pos, depth, true, None) }
    fn merge_bytes_dont_enforce(&mut self, data: &[u8], depth: u32) -> Result<(), ParseError> { if data.is_empty() { return Ok(()); } let mut pos = 0; let mut wire = None; self.merge_inner(data, &mut wire, &mut pos, depth, false, None) }
    fn merge_group(&mut self, data: &[u8], wire: &mut Option<pbrs::rt::Wire>, pos: &mut usize, num: u32, depth: u32) -> Result<(), ParseError> { self.merge_inner(data, wire, pos, depth, false, Some(num)) }
    fn merge_inner(&mut self, data: &[u8], wire: &mut Option<pbrs::rt::Wire>, pos: &mut usize, depth: u32, enforce: bool, until: Option<u32>) -> Result<(), ParseError> {
        if depth > pbrs::RECURSION_LIMIT { return Err(ParseError::new("recursion limit exceeded")); } let _ = enforce;
        self.cached_size.dirty();
        while *pos < data.len() {
            let (n, w) = pbrs::rt::decode_tag(data, pos)?;
            if let Some(g) = until { if w == pbrs::rt::WIRE_EGROUP { if n != g { return Err(ParseError::new("mismatched end-group")); } return Ok(()); } }
            if n == 1 {
                match w {
                pbrs::rt::WIRE_VARINT => {
                    self.id = pbrs::rt::decode_varint(data, pos)? as i32;
                    }
                    _ => self.unknown.fields.push(pbrs::rt::capture_unknown(data, pos, n, w)?),
                }
                continue;
            }
            self.merge_heavy(n, w, data, wire, pos, depth)?;
        }
        if until.is_some() { return Err(ParseError::new("truncated group")); }
        if enforce { self.check_required()?; }
        Ok(())
    }
    #[inline(never)]
    fn merge_heavy(&mut self, n: u32, w: u32, data: &[u8], wire: &mut Option<pbrs::rt::Wire>, pos: &mut usize, depth: u32) -> Result<(), ParseError> {
        match n {
            2 => match w {
                pbrs::rt::WIRE_LEN => {
                    let (s, e) = pbrs::rt::read_len_span(data, pos)?; self.cold_mut().name = pbrs::rt::LazyStr::from_parse_span(wire, data, s, e)?;
                    }
                _ => self.unknown.fields.push(pbrs::rt::capture_unknown(data, pos, n, w)?),
            }
            3 => match w {
                pbrs::rt::WIRE_LEN => {
                    let (s, e) = pbrs::rt::read_len_span(data, pos)?; self.cold_mut().email = Some(Box::new(pbrs::rt::LazyStr::from_parse_span(wire, data, s, e)?));
                    }
                _ => self.unknown.fields.push(pbrs::rt::capture_unknown(data, pos, n, w)?),
            }
            4 => match w {
                pbrs::rt::WIRE_LEN => { let (s, e) = pbrs::rt::read_len_span(data, pos)?; let rest = data.len().saturating_sub(e); if rest > 2 { self.cold_mut().tags.reserve(1 + rest / (e - s + 2).max(1)); } self.cold_mut().tags.push(pbrs::rt::LazyStr::from_parse_span(wire, data, s, e)?); while *pos < data.len() { let save = *pos; match pbrs::rt::decode_tag(data, pos) { Ok((n2, w2)) if n2 == 4 && w2 == pbrs::rt::WIRE_LEN => { let (s, e) = pbrs::rt::read_len_span(data, pos)?; self.cold_mut().tags.push(pbrs::rt::LazyStr::from_parse_span(wire, data, s, e)?); } Ok(_) => { *pos = save; break; } Err(e) => return Err(e), } } }
                _ => self.unknown.fields.push(pbrs::rt::capture_unknown(data, pos, n, w)?),
            }
            5 => match w {
                pbrs::rt::WIRE_LEN => {
                    let (s, e) = pbrs::rt::read_len_span(data, pos)?;
                    let (kk, vv) = decode_map_entry_Person_scores_5(&pbrs::rt::Wire::ensure(wire, data).window(s, e), depth + 1)?;
                    self.cold_mut().scores.push_entry(kk, vv);
                }
                _ => self.unknown.fields.push(pbrs::rt::capture_unknown(data, pos, n, w)?),
            }
            6 => match w {
                pbrs::rt::WIRE_LEN => {
                    let (s, e) = pbrs::rt::read_len_span(data, pos)?;
                    if self.cold_mut().address.is_some() { let mut ip = 0; let mut sw = None; self.cold_mut().address.get_or_insert().merge_inner(&data[s..e], &mut sw, &mut ip, depth + 1, true, None)?; } else { let mut ip = 0; Address::validate_inner(&pbrs::rt::Wire::ensure(wire, data).window(s, e), &mut ip, depth + 1)?; self.cold_mut().address = pbrs::rt::LazyMsg::from_wire(pbrs::rt::Wire::ensure(wire, data).window(s, e)); }
                    }
                _ => self.unknown.fields.push(pbrs::rt::capture_unknown(data, pos, n, w)?),
            }
            16 => match w {
                pbrs::rt::WIRE_LEN => {
                    let (s, e) = pbrs::rt::read_len_span(data, pos)?;
                    let (kk, vv) = decode_map_entry_Person_extras_16(&pbrs::rt::Wire::ensure(wire, data).window(s, e), depth + 1)?;
                    self.cold_mut().extras.push_entry(kk, vv);
                }
                _ => self.unknown.fields.push(pbrs::rt::capture_unknown(data, pos, n, w)?),
            }
            _ => self.unknown.fields.push(pbrs::rt::capture_unknown(data, pos, n, w)?),
        }
        Ok(())
    }
    fn validate_inner(wire: &pbrs::rt::Wire, pos: &mut usize, depth: u32) -> Result<(), ParseError> { Self::validate_until(wire, pos, depth, None) }
    fn validate_until(wire: &pbrs::rt::Wire, pos: &mut usize, depth: u32, until: Option<u32>) -> Result<(), ParseError> {
        if depth > pbrs::RECURSION_LIMIT { return Err(ParseError::new("recursion limit exceeded")); }
        let data = wire.as_slice();
        while *pos < data.len() {
            let (n, w) = pbrs::rt::decode_tag(data, pos)?;
            if let Some(g) = until { if w == pbrs::rt::WIRE_EGROUP { if n != g { return Err(ParseError::new("mismatched end-group")); } return Ok(()); } }
            match n {
            1 => match w {
                pbrs::rt::WIRE_VARINT => { let _ = pbrs::rt::decode_varint(data, pos)? as i32; }
                _ => pbrs::rt::skip_field(data, pos, w)?,
            }
            2 => match w {
                pbrs::rt::WIRE_LEN => { let (s, e) = pbrs::rt::read_len_span(data, pos)?; pbrs::rt::require_utf8(&data[s..e])?; }
                _ => pbrs::rt::skip_field(data, pos, w)?,
            }
            3 => match w {
                pbrs::rt::WIRE_LEN => { let (s, e) = pbrs::rt::read_len_span(data, pos)?; pbrs::rt::require_utf8(&data[s..e])?; }
                _ => pbrs::rt::skip_field(data, pos, w)?,
            }
            4 => match w {
                pbrs::rt::WIRE_LEN => { let (s, e) = pbrs::rt::read_len_span(data, pos)?; pbrs::rt::require_utf8(&data[s..e])?; }
                _ => pbrs::rt::skip_field(data, pos, w)?,
            }
            5 => match w {
                pbrs::rt::WIRE_LEN => { let (s, e) = pbrs::rt::read_len_span(data, pos)?; let mut ip = 0; let w = wire.window(s, e); let d = w.as_slice(); while ip < d.len() { let (_, ww) = pbrs::rt::decode_tag(d, &mut ip)?; pbrs::rt::skip_field(d, &mut ip, ww)?; } }
                _ => pbrs::rt::skip_field(data, pos, w)?,
            }
            6 => match w {
                pbrs::rt::WIRE_LEN => { let (s, e) = pbrs::rt::read_len_span(data, pos)?; let mut ip = 0; Address::validate_inner(&wire.window(s, e), &mut ip, depth + 1)?; }
                _ => pbrs::rt::skip_field(data, pos, w)?,
            }
            16 => match w {
                pbrs::rt::WIRE_LEN => { let (s, e) = pbrs::rt::read_len_span(data, pos)?; let mut ip = 0; let w = wire.window(s, e); let d = w.as_slice(); while ip < d.len() { let (_, ww) = pbrs::rt::decode_tag(d, &mut ip)?; pbrs::rt::skip_field(d, &mut ip, ww)?; } }
                _ => pbrs::rt::skip_field(data, pos, w)?,
            }
                _ => pbrs::rt::skip_field(data, pos, w)?,
            }
        }
        if until.is_some() { return Err(ParseError::new("truncated group")); }
        Ok(())
    }
    fn compute_size(&self) -> u64 {
        if let Some(n) = self.cached_size.get() { return n; }
        let mut n = self.unknown.encoded_len();
        if self.id != 0 { n += pbrs::rt::tag_len(1, pbrs::rt::WIRE_VARINT) + pbrs::rt::varint_len((self.id) as u64); }
        if let Some(c) = self.cold.as_deref() {
        if !c.name.is_empty() { n += pbrs::rt::key_len_value_len(2, c.name.as_bytes().len() as u64); }
        if let Some(s) = &c.email { n += pbrs::rt::key_len_value_len(3, s.as_bytes().len() as u64); }
        for t in c.tags.iter() { n += pbrs::rt::key_len_value_len(4, t.as_bytes().len() as u64); }
        if !c.scores.is_empty() {
        for (k, v) in c.scores.pairs() {
            let inner = pbrs::rt::key_len_value_len(1, k.as_bytes().len() as u64) + pbrs::rt::tag_len(2, pbrs::rt::WIRE_VARINT) + pbrs::rt::varint_len((*v) as u64);
            n += pbrs::rt::key_len_value_len(5, inner);
        }
        }
        if let Some(p) = c.address.wire_bytes() { n += pbrs::rt::key_len_value_len(6, p.len() as u64); } else if let Some(m) = c.address.as_deref() { n += pbrs::rt::key_len_value_len(6, m.compute_size()); }
        if !c.extras.is_empty() {
        for (k, v) in c.extras.pairs() {
            let inner = pbrs::rt::key_len_value_len(1, k.as_bytes().len() as u64) + pbrs::rt::tag_len(2, pbrs::rt::WIRE_VARINT) + pbrs::rt::varint_len((*v) as u64);
            n += pbrs::rt::key_len_value_len(16, inner);
        }
        }
        }
        self.cached_size.set(n);
        n
    }
    fn write_to(&self, out: &mut impl pbrs::rt::WireOut) {
        if self.id != 0 { pbrs::rt::encode_tag(out, 1, pbrs::rt::WIRE_VARINT); pbrs::rt::encode_varint(out, self.id as u64); }
        if let Some(c) = self.cold.as_deref() {
        if !c.name.is_empty() { pbrs::rt::encode_len_field(out, 2, c.name.as_bytes()); }
        if let Some(s) = &c.email { pbrs::rt::encode_len_field(out, 3, s.as_bytes()); }
        for t in c.tags.iter() { pbrs::rt::encode_len_field(out, 4, t.as_bytes()); }
        if !c.scores.is_empty() {
        for (k, v) in c.scores.pairs() {
            let inner = pbrs::rt::key_len_value_len(1, k.as_bytes().len() as u64) + pbrs::rt::tag_len(2, pbrs::rt::WIRE_VARINT) + pbrs::rt::varint_len((*v) as u64);
            pbrs::rt::encode_tag(out, 5, pbrs::rt::WIRE_LEN); pbrs::rt::encode_varint(out, inner);
            pbrs::rt::encode_len_field(out, 1, k.as_bytes());
            pbrs::rt::encode_tag(out, 2, pbrs::rt::WIRE_VARINT); pbrs::rt::encode_varint(out, *v as u64);
        }
        }
        if let Some(p) = c.address.wire_bytes() { pbrs::rt::encode_len_header(out, 6, p.len() as u64); out.extend_from_slice(p); } else if let Some(m) = c.address.as_deref() { pbrs::rt::encode_len_header(out, 6, m.compute_size()); m.write_to(out); }
        if !c.extras.is_empty() {
        for (k, v) in c.extras.pairs() {
            let inner = pbrs::rt::key_len_value_len(1, k.as_bytes().len() as u64) + pbrs::rt::tag_len(2, pbrs::rt::WIRE_VARINT) + pbrs::rt::varint_len((*v) as u64);
            pbrs::rt::encode_tag(out, 16, pbrs::rt::WIRE_LEN); pbrs::rt::encode_varint(out, inner);
            pbrs::rt::encode_len_field(out, 1, k.as_bytes());
            pbrs::rt::encode_tag(out, 2, pbrs::rt::WIRE_VARINT); pbrs::rt::encode_varint(out, *v as u64);
        }
        }
        }
        self.unknown.encode(out);
    }
    pub fn to_json(&self) -> Result<String, SerializeError> {
        Ok(self.to_json_value()?.to_string())
    }
    pub fn from_json(json: &str) -> Result<Self, ParseError> { Self::from_json_ignore(json, false) }
    pub fn from_json_ignore(json: &str, ignore: bool) -> Result<Self, ParseError> {
        let v = pbrs::json::parse(json)?;
        Self::from_json_value(&v, ignore)
    }
    fn to_json_value(&self) -> Result<pbrs::json::Json, SerializeError> {
        let mut map = pbrs::json::JsonMap::new();
        if self.id() != 0 {
            let _ = map.insert("id".into(), pbrs::json::int32(self.id()));
        }
        if !self.name().as_bytes().is_empty() {
            let _ = map.insert("name".into(), pbrs::json::string(self.name().as_bytes()));
        }
        if self.has_email() {
            let _ = map.insert("email".into(), pbrs::json::string(self.email().as_bytes()));
        }
        if !self.tags().is_empty() {
            let mut arr = Vec::new();
            for item in self.tags() {
                arr.push(pbrs::json::string(item.as_bytes()));
            }
            let _ = map.insert("tags".into(), pbrs::json::Json::Array(arr));
        }
        if !self.scores().is_empty() {
            let mut obj = pbrs::json::JsonMap::new();
            for (k, v) in self.scores() {
                let _ = obj.insert(String::from_utf8_lossy(k.as_bytes()).into_owned(), pbrs::json::int32(v));
            }
            let _ = map.insert("scores".into(), pbrs::json::Json::Object(obj));
        }
        if self.has_address() {
            let _ = map.insert("address".into(), self.address().to_json_value()?);
        }
        if !self.extras().is_empty() {
            let mut obj = pbrs::json::JsonMap::new();
            for (k, v) in self.extras() {
                let _ = obj.insert(String::from_utf8_lossy(k.as_bytes()).into_owned(), pbrs::json::int32(v));
            }
            let _ = map.insert("extras".into(), pbrs::json::Json::Object(obj));
        }
        Ok(pbrs::json::Json::Object(map))
    }
    fn from_json_value(v: &pbrs::json::Json, ignore: bool) -> Result<Self, ParseError> {
        let obj = v.as_object().ok_or_else(|| ParseError::new("json message must be an object"))?;
        let mut msg = Self::new();
        let mut seen = std::collections::BTreeSet::<u32>::new();
        for (key, val) in obj {
            match key.as_str() {
                "id" => {
                    if !seen.insert(1u32) { return Err(ParseError::owned(format!("duplicate json field {key}"))); }
                    if val.is_null() { continue; }
                    msg.set_id(pbrs::json::as_i32(val)?);
                }
                "name" => {
                    if !seen.insert(2u32) { return Err(ParseError::owned(format!("duplicate json field {key}"))); }
                    if val.is_null() { continue; }
                    msg.set_name(pbrs::json::as_str(val)?);
                }
                "email" => {
                    if !seen.insert(3u32) { return Err(ParseError::owned(format!("duplicate json field {key}"))); }
                    if val.is_null() { continue; }
                    msg.set_email(pbrs::json::as_str(val)?);
                }
                "tags" => {
                    if !seen.insert(4u32) { return Err(ParseError::owned(format!("duplicate json field {key}"))); }
                    if val.is_null() { continue; }
                    let arr = val.as_array().ok_or_else(|| ParseError::new("json repeated must be an array"))?;
                    for item in arr {
                        msg.tags_mut().push(pbrs::json::as_str(item)?);
                    }
                }
                "scores" => {
                    if !seen.insert(5u32) { return Err(ParseError::owned(format!("duplicate json field {key}"))); }
                    if val.is_null() { continue; }
                    let obj = val.as_object().ok_or_else(|| ParseError::new("json map must be an object"))?;
                    for (k, v) in obj {
                        msg.scores_mut().insert(k.as_str(), pbrs::json::as_i32(v)?);
                    }
                }
                "address" => {
                    if !seen.insert(6u32) { return Err(ParseError::owned(format!("duplicate json field {key}"))); }
                    if val.is_null() { continue; }
                    msg.set_address(Address::from_json_value(val, ignore)?);
                }
                "extras" => {
                    if !seen.insert(16u32) { return Err(ParseError::owned(format!("duplicate json field {key}"))); }
                    if val.is_null() { continue; }
                    let obj = val.as_object().ok_or_else(|| ParseError::new("json map must be an object"))?;
                    for (k, v) in obj {
                        msg.extras_mut().insert(k.as_str(), pbrs::json::as_i32(v)?);
                    }
                }
                _ => {
                    if !ignore { return Err(ParseError::owned(format!("unknown json field {key}"))); }
                }
            }
        }
        Ok(msg)
    }
    pub fn to_text(&self) -> Result<String, SerializeError> {
        let mut out = String::new();
        self.write_text(&mut out, 0)?;
        Ok(out)
    }
    pub fn to_text_with_unknown(&self) -> Result<String, SerializeError> {
        let mut out = String::new();
        self.write_text(&mut out, 0)?;
        pbrs::text::write_unknown_fields(&self.unknown, &mut out, 0);
        Ok(out)
    }
    pub fn from_text(text: &str) -> Result<Self, ParseError> {
        Self::from_text_value(&pbrs::text::parse(text)?)
    }
    fn write_text(&self, out: &mut String, indent: usize) -> Result<(), SerializeError> {
        if self.id() != 0 {
            pbrs::text::write_named_int32(out, indent, "id", self.id());
        }
        if !self.name().as_bytes().is_empty() {
            pbrs::text::write_named_string(out, indent, "name", self.name().as_bytes());
        }
        if self.has_email() {
            pbrs::text::write_named_string(out, indent, "email", self.email().as_bytes());
        }
        for item in self.tags() {
            pbrs::text::write_named_string(out, indent, "tags", item.as_bytes());
        }
        if !self.scores().is_empty() {
            let mut items: Vec<_> = self.scores().iter().collect();
            items.sort_by(|(a, _), (b, _)| a.as_bytes().cmp(b.as_bytes()));
            for (k, v) in items {
                pbrs::text::write_map_entry(out, indent, "scores", |out| { pbrs::text::write_bytes_lit(k.as_bytes(), out); }, |out| { out.push_str(&v.to_string()); Ok(()) }, false)?;
            }
        }
        if self.has_address() {
            pbrs::text::pad(out, indent);
            out.push_str("address");
            out.push_str(" {\n");
            self.address().write_text(out, indent + 2)?;
            pbrs::text::pad(out, indent);
            out.push_str("}\n");
        }
        if !self.extras().is_empty() {
            let mut items: Vec<_> = self.extras().iter().collect();
            items.sort_by(|(a, _), (b, _)| a.as_bytes().cmp(b.as_bytes()));
            for (k, v) in items {
                pbrs::text::write_map_entry(out, indent, "extras", |out| { pbrs::text::write_bytes_lit(k.as_bytes(), out); }, |out| { out.push_str(&v.to_string()); Ok(()) }, false)?;
            }
        }
        Ok(())
    }
    fn from_text_value(fields: &[(String, pbrs::text::TextValue)]) -> Result<Self, ParseError> {
        let mut msg = Self::new();
        for (key, val) in fields {
            match key.as_str() {
                "id" => {
                    msg.set_id(val.as_i32()?);
                }
                "name" => {
                    msg.set_name(val.as_str()?);
                }
                "email" => {
                    msg.set_email(val.as_str()?);
                }
                "tags" => {
                    if let Some(items) = val.as_list() {
                        for item in items {
                            msg.tags_mut().push(item.as_str()?);
                        }
                    } else {
                        msg.tags_mut().push(val.as_str()?);
                    }
                }
                "scores" => {
                    let entry = val.as_message()?;
                    let mut k = "";
                    let mut v = 0i32;
                    for (ek, ev) in entry {
                        match ek.as_str() {
                            "key" => k = ev.as_str()?,
                            "value" => v = ev.as_i32()?,
                            _ => return Err(ParseError::owned(format!("unknown field {ek}"))),
                        }
                    }
                    msg.scores_mut().insert(k, v);
                }
                "address" => {
                    msg.address_mut().merge_from(Address::from_text_value(val.as_message()?)?);
                }
                "extras" => {
                    let entry = val.as_message()?;
                    let mut k = "";
                    let mut v = 0i32;
                    for (ek, ev) in entry {
                        match ek.as_str() {
                            "key" => k = ev.as_str()?,
                            "value" => v = ev.as_i32()?,
                            _ => return Err(ParseError::owned(format!("unknown field {ek}"))),
                        }
                    }
                    msg.extras_mut().insert(k, v);
                }
                _ => return Err(ParseError::owned(format!("unknown field {key}"))),
            }
        }
        Ok(msg)
    }
}
pbrs::impl_typed_message!(Person, PersonView, PersonMut);
fn decode_map_entry_Person_scores_5(wire: &pbrs::rt::Wire, depth: u32) -> Result<(pbrs::rt::LazyStr, i32), ParseError> {
    let _ = depth; let data = wire.as_slice(); let mut key = pbrs::rt::LazyStr::default(); let mut val = i32::default(); let mut pos = 0;
    while pos < data.len() { let (n, w) = pbrs::rt::decode_tag(data, &mut pos)?; match (n, w) {
        (1, pbrs::rt::WIRE_LEN) => { let (s, e) = pbrs::rt::read_len_span(data, &mut pos)?; pbrs::rt::require_utf8(&data[s..e])?; key = pbrs::rt::LazyStr::from_span(wire, s, e); },
        (2, pbrs::rt::WIRE_VARINT) => val = pbrs::rt::decode_varint(data, &mut pos)? as i32,
        _ => pbrs::rt::skip_field(data, &mut pos, w)?,
    } }
    Ok((key, val))
}
fn decode_map_entry_Person_extras_16(wire: &pbrs::rt::Wire, depth: u32) -> Result<(pbrs::rt::LazyStr, i32), ParseError> {
    let _ = depth; let data = wire.as_slice(); let mut key = pbrs::rt::LazyStr::default(); let mut val = i32::default(); let mut pos = 0;
    while pos < data.len() { let (n, w) = pbrs::rt::decode_tag(data, &mut pos)?; match (n, w) {
        (1, pbrs::rt::WIRE_LEN) => { let (s, e) = pbrs::rt::read_len_span(data, &mut pos)?; pbrs::rt::require_utf8(&data[s..e])?; key = pbrs::rt::LazyStr::from_span(wire, s, e); },
        (2, pbrs::rt::WIRE_VARINT) => val = pbrs::rt::decode_varint(data, &mut pos)? as i32,
        _ => pbrs::rt::skip_field(data, &mut pos, w)?,
    } }
    Ok((key, val))
}
}
#[allow(unused_imports, reason = "generated by protoc-gen-pbrs")]
pub use __gen::*;

}

fn median_ns<F: FnMut()>(samples: usize, iters: u32, mut f: F) -> f64 {
    let mut xs: Vec<f64> = (0..samples).map(|_| bench_ns(iters, &mut f)).collect();
    xs.sort_by(|a, b| a.partial_cmp(b).unwrap());
    xs[samples / 2]
}

/// 40k iters of 5 MiB is a multi-minute memcpy loop. Scale the timer down.
fn timer_budget(payload: usize) -> (u32, usize) {
    if payload >= 3_000_000 {
        (40, 7)
    } else if payload >= 500_000 {
        (120, 9)
    } else {
        (40_000, 15)
    }
}

fn bench_ns<F: FnMut()>(iters: u32, mut f: F) -> f64 {
    for _ in 0..iters / 10 {
        f();
    }
    let t = Instant::now();
    for _ in 0..iters {
        std::hint::black_box(f());
    }
    t.elapsed().as_secs_f64() * 1e9 / f64::from(iters)
}

fn tat_populated() -> TestAllTypesProto3 {
    let mut nested = NestedMessage::new();
    nested.set_a(9);
    let mut m = TestAllTypesProto3::new();
    m.set_optional_int32(7);
    m.set_optional_int64(1 << 40);
    m.set_optional_uint32(99);
    m.set_optional_string("ada lovelace");
    m.set_optional_bytes(&b"notes"[..]);
    m.set_optional_nested_message(nested);
    for i in 0..8 {
        m.repeated_int32_mut().push(i);
        m.packed_int32_mut().push(i * 3);
    }
    for i in 0..4 {
        m.map_int32_int32_mut().insert(i, i * i);
    }
    m
}

fn tat_packed() -> TestAllTypesProto3 {
    let mut m = TestAllTypesProto3::new();
    for i in 0..256 {
        m.packed_int32_mut().push(i);
    }
    m
}

fn tat_map() -> TestAllTypesProto3 {
    let mut m = TestAllTypesProto3::new();
    for i in 0..64 {
        m.map_int32_int32_mut().insert(i, i * i);
    }
    m
}

fn tat_nested() -> TestAllTypesProto3 {
    let mut inner = TestAllTypesProto3::new();
    inner.set_optional_int32(1);
    for _ in 0..8 {
        let mut outer = TestAllTypesProto3::new();
        outer.set_recursive_message(inner);
        inner = outer;
    }
    inner
}

fn tat_strings() -> TestAllTypesProto3 {
    let mut m = TestAllTypesProto3::new();
    m.set_optional_string("the quick brown fox jumps over the lazy dog");
    m.set_optional_string_piece("string piece payload for encode/decode");
    m.set_optional_cord("cord-shaped string used as a singular field");
    for s in ["alpha", "beta", "gamma", "delta"] {
        m.repeated_string_mut().push(s);
    }
    m
}

fn tat_unpacked() -> TestAllTypesProto3 {
    let mut m = TestAllTypesProto3::new();
    for i in 0..256 {
        m.unpacked_int32_mut().push(i);
    }
    m
}

fn tat_packed_fixed() -> TestAllTypesProto3 {
    let mut m = TestAllTypesProto3::new();
    for i in 0..256u32 {
        m.packed_fixed32_mut().push(i);
    }
    m
}

fn tat_packed_fixed64() -> TestAllTypesProto3 {
    let mut m = TestAllTypesProto3::new();
    for i in 0..256u64 {
        m.packed_fixed64_mut().push(i);
    }
    m
}

fn tat_packed_float() -> TestAllTypesProto3 {
    let mut m = TestAllTypesProto3::new();
    for i in 0..256 {
        m.packed_float_mut().push(i as f32);
    }
    m
}

fn tat_unpacked_fixed() -> TestAllTypesProto3 {
    let mut m = TestAllTypesProto3::new();
    for i in 0..256u32 {
        m.unpacked_fixed32_mut().push(i);
    }
    m
}

fn tat_oneof() -> TestAllTypesProto3 {
    let mut m = TestAllTypesProto3::new();
    m.set_oneof_string("oneof-string-payload");
    m
}

fn tat_repeated_nested() -> TestAllTypesProto3 {
    let mut m = TestAllTypesProto3::new();
    for i in 0..8 {
        let mut n = NestedMessage::new();
        n.set_a(i);
        m.repeated_nested_message_mut().push(n);
    }
    m
}

fn tat_bytes() -> TestAllTypesProto3 {
    let mut m = TestAllTypesProto3::new();
    m.set_optional_bytes(&b"optional-bytes-payload-0123456789"[..]);
    for i in 0..8u8 {
        m.repeated_bytes_mut().push(vec![i; 32]);
    }
    m
}

fn tat_bytes_n(n: usize) -> TestAllTypesProto3 {
    let mut m = TestAllTypesProto3::new();
    m.set_optional_bytes(vec![0x5a; n]);
    m
}

fn tat_packed_fixed_n(n: usize) -> TestAllTypesProto3 {
    let mut m = TestAllTypesProto3::new();
    m.set_packed_fixed32(0..n as u32);
    m
}

fn tat_scalars() -> TestAllTypesProto3 {
    let mut m = TestAllTypesProto3::new();
    m.set_optional_bool(true);
    m.set_optional_float(1.5);
    m.set_optional_nested_enum(1);
    for i in 0..64 {
        m.packed_bool_mut().push(i % 2 == 0);
    }
    m
}

fn prost_of(m: &TestAllTypesProto3) -> prost_tat::TestAllTypesProto3 {
    let nested = m.optional_nested_message_opt().map(|n| {
        Box::new(prost_tat::test_all_types_proto3::NestedMessage {
            a: n.a(),
            ..Default::default()
        })
    });
    let rec = m.recursive_message_opt().map(|r| Box::new(prost_of(r)));
    prost_tat::TestAllTypesProto3 {
        optional_int32: m.optional_int32(),
        optional_int64: m.optional_int64(),
        optional_uint32: m.optional_uint32(),
        optional_string: m.optional_string().to_str().unwrap_or("").to_string(),
        optional_bytes: m.optional_bytes().to_vec(),
        optional_nested_message: nested,
        optional_string_piece: m.optional_string_piece().to_str().unwrap_or("").to_string(),
        optional_cord: m.optional_cord().to_str().unwrap_or("").to_string(),
        recursive_message: rec,
        repeated_int32: m.repeated_int32().iter().collect(),
        map_int32_int32: m.map_int32_int32().iter().map(|(k, v)| (k, v)).collect(),
        packed_int32: m.packed_int32().iter().collect(),
        repeated_string: m
            .repeated_string()
            .iter()
            .map(|s| s.as_view().to_str().unwrap_or("").to_string())
            .collect(),
        optional_bool: m.optional_bool(),
        optional_float: m.optional_float(),
        optional_nested_enum: i32::from(m.optional_nested_enum()),
        repeated_bytes: m
            .repeated_bytes()
            .iter()
            .map(|b| b.as_bytes().to_vec())
            .collect(),
        packed_fixed32: m.packed_fixed32().iter().collect(),
        packed_fixed64: m.packed_fixed64().iter().collect(),
        packed_float: m.packed_float().iter().collect(),
        packed_bool: m.packed_bool().iter().collect(),
        unpacked_int32: m.unpacked_int32().iter().collect(),
        unpacked_fixed32: m.unpacked_fixed32().iter().collect(),
        repeated_nested_message: m
            .repeated_nested_message()
            .iter()
            .map(|n| prost_tat::test_all_types_proto3::NestedMessage {
                a: n.a(),
                ..Default::default()
            })
            .collect(),
        oneof_field: m
            .oneof_uint32_opt()
            .map(prost_tat::test_all_types_proto3::OneofField::OneofUint32)
            .or_else(|| {
                m.oneof_string_opt().map(|s| {
                    prost_tat::test_all_types_proto3::OneofField::OneofString(
                        s.to_str().unwrap_or("").to_string(),
                    )
                })
            }),
        ..Default::default()
    }
}

fn v4_of(m: &TestAllTypesProto3) -> v4_tat::TestAllTypesProto3 {
    let mut v = v4_tat::TestAllTypesProto3::new();
    v.set_optional_int32(m.optional_int32());
    v.set_optional_int64(m.optional_int64());
    v.set_optional_uint32(m.optional_uint32());
    v.set_optional_string(m.optional_string().to_str().unwrap_or(""));
    v.set_optional_bytes(m.optional_bytes());
    if let Some(n) = m.optional_nested_message_opt() {
        v.optional_nested_message_mut().set_a(n.a());
    }
    if let Some(s) = m
        .optional_string_piece()
        .to_str()
        .ok()
        .filter(|s| !s.is_empty())
    {
        v.set_optional_string_piece(s);
    }
    if let Some(s) = m.optional_cord().to_str().ok().filter(|s| !s.is_empty()) {
        v.set_optional_cord(s);
    }
    if let Some(r) = m.recursive_message_opt() {
        v.set_recursive_message(v4_of(r));
    }
    for i in m.repeated_int32().iter() {
        v.repeated_int32_mut().push(i);
    }
    for (k, val) in m.map_int32_int32().iter() {
        v.map_int32_int32_mut().insert(k, val);
    }
    for i in m.packed_int32().iter() {
        v.packed_int32_mut().push(i);
    }
    for s in m.repeated_string().iter() {
        v.repeated_string_mut()
            .push(s.as_view().to_str().unwrap_or(""));
    }
    v.set_optional_bool(m.optional_bool());
    v.set_optional_float(m.optional_float());
    v.set_optional_nested_enum(v4_tat::test_all_types_proto3::NestedEnum::from(i32::from(
        m.optional_nested_enum(),
    )));
    for b in m.repeated_bytes().iter() {
        v.repeated_bytes_mut().push(b.as_bytes());
    }
    for i in m.packed_fixed32().iter() {
        v.packed_fixed32_mut().push(i);
    }
    for i in m.packed_fixed64().iter() {
        v.packed_fixed64_mut().push(i);
    }
    for i in m.packed_float().iter() {
        v.packed_float_mut().push(i);
    }
    for i in m.packed_bool().iter() {
        v.packed_bool_mut().push(i);
    }
    for i in m.unpacked_int32().iter() {
        v.unpacked_int32_mut().push(i);
    }
    for i in m.unpacked_fixed32().iter() {
        v.unpacked_fixed32_mut().push(i);
    }
    for n in m.repeated_nested_message().iter() {
        let mut inner = v4_tat::test_all_types_proto3::NestedMessage::new();
        inner.set_a(n.a());
        v.repeated_nested_message_mut().push(inner);
    }
    if let Some(x) = m.oneof_uint32_opt() {
        v.set_oneof_uint32(x);
    } else if let Some(s) = m.oneof_string_opt() {
        v.set_oneof_string(s.to_str().unwrap_or(""));
    }
    v
}

fn buffa_of(m: &TestAllTypesProto3) -> BuffaTat {
    let nested = m.optional_nested_message_opt().map(|n| BuffaNested {
        a: n.a(),
        ..Default::default()
    });
    BuffaTat {
        optional_int32: m.optional_int32(),
        optional_int64: m.optional_int64(),
        optional_uint32: m.optional_uint32(),
        optional_string: m.optional_string().to_str().unwrap_or("").to_string(),
        optional_bytes: m.optional_bytes().to_vec(),
        optional_nested_message: nested.into(),
        optional_string_piece: m.optional_string_piece().to_str().unwrap_or("").to_string(),
        optional_cord: m.optional_cord().to_str().unwrap_or("").to_string(),
        recursive_message: m.recursive_message_opt().map(buffa_of).into(),
        repeated_int32: m.repeated_int32().iter().collect(),
        map_int32_int32: m.map_int32_int32().iter().map(|(k, v)| (k, v)).collect(),
        packed_int32: m.packed_int32().iter().collect(),
        repeated_string: m
            .repeated_string()
            .iter()
            .map(|s| s.as_view().to_str().unwrap_or("").to_string())
            .collect(),
        optional_bool: m.optional_bool(),
        optional_float: m.optional_float(),
        optional_nested_enum: i32::from(m.optional_nested_enum()).into(),
        repeated_bytes: m
            .repeated_bytes()
            .iter()
            .map(|b| b.as_bytes().to_vec())
            .collect(),
        packed_fixed32: m.packed_fixed32().iter().collect(),
        packed_fixed64: m.packed_fixed64().iter().collect(),
        packed_float: m.packed_float().iter().collect(),
        packed_bool: m.packed_bool().iter().collect(),
        unpacked_int32: m.unpacked_int32().iter().collect(),
        unpacked_fixed32: m.unpacked_fixed32().iter().collect(),
        repeated_nested_message: m
            .repeated_nested_message()
            .iter()
            .map(|n| BuffaNested {
                a: n.a(),
                ..Default::default()
            })
            .collect(),
        oneof_field: m
            .oneof_uint32_opt()
            .map(buffa_tat::protobuf_test_messages::proto3::__buffa::oneof::test_all_types_proto3::OneofField::OneofUint32)
            .or_else(|| {
                m.oneof_string_opt().map(|s| {
                    buffa_tat::protobuf_test_messages::proto3::__buffa::oneof::test_all_types_proto3::OneofField::OneofString(
                        s.to_str().unwrap_or("").to_string(),
                    )
                })
            }),
        ..Default::default()
    }
}

fn person_ours() -> Person {
    let mut addr = Address::new();
    addr.set_city("nyc");
    let mut p = Person::new();
    p.set_id(7);
    p.set_name("ada lovelace");
    p.set_email("ada@example.com");
    p.tags_mut().push("math");
    p.tags_mut().push("eng");
    p.scores_mut().insert("notes", 12);
    p.set_address(addr);
    p
}

#[derive(Clone, PartialEq, Message)]
struct ProstAddress {
    #[prost(string, tag = "1")]
    city: String,
}

#[derive(Clone, PartialEq, Message)]
struct ProstPerson {
    #[prost(int32, tag = "1")]
    id: i32,
    #[prost(string, tag = "2")]
    name: String,
    #[prost(string, optional, tag = "3")]
    email: Option<String>,
    #[prost(string, repeated, tag = "4")]
    tags: Vec<String>,
    #[prost(map = "string, int32", tag = "5")]
    scores: std::collections::HashMap<String, i32>,
    #[prost(message, optional, tag = "6")]
    address: Option<ProstAddress>,
}

struct Case {
    name: &'static str,
    layout: &'static str,
    ours_enc: f64,
    ours_fresh_enc: Option<f64>,
    ours_mutated_enc: Option<f64>,
    prost_enc: f64,
    v4_enc: f64,
    buffa_enc: f64,
    ours_dec: f64,
    prost_dec: f64,
    v4_dec: f64,
    buffa_dec: f64,
    buffa_view: Option<f64>,
    payload: usize,
    ours_def: Option<f64>,
    iters: u32,
    samples: usize,
}

fn run_tat(name: &'static str, msg: TestAllTypesProto3, iters: u32) -> Case {
    let bytes = pbrs::Serialize::serialize(&msg).expect("ours encode");
    let (iters, samples) = if bytes.len() >= 500_000 {
        timer_budget(bytes.len())
    } else {
        (iters, 15)
    };
    eprintln!(
        "bench {name}: payload={} iters={iters} samples={samples}",
        bytes.len()
    );
    let prost_msg = prost_of(&msg);
    let v4_msg = v4_of(&msg);
    let buffa_msg = buffa_of(&msg);
    let ours_def = if name == "tat_populated" {
        Some(median_ns(samples, iters, || {
            let _ = TestAllTypesProto3::new();
        }))
    } else {
        None
    };
    let ours_dec = median_ns(samples, iters, || {
        let _ = TestAllTypesProto3::parse(&bytes).unwrap();
    });
    let ours_enc = median_ns(samples, iters, || {
        let _ = pbrs::Serialize::serialize(&msg).unwrap();
    });
    let parse_and_enc = median_ns(samples, iters, || {
        let fresh = TestAllTypesProto3::parse(&bytes).unwrap();
        let _ = pbrs::Serialize::serialize(&fresh).unwrap();
    });
    let ours_fresh_enc = Some((parse_and_enc - ours_dec).max(ours_enc));
    let mut mutated = msg.clone();
    let ours_mutated_enc = Some(median_ns(samples, iters, || {
        mutated.set_optional_int32(42);
        let _ = pbrs::Serialize::serialize(&mutated).unwrap();
    }));
    Case {
        name,
        layout: "generated",
        payload: bytes.len(),
        ours_enc,
        ours_fresh_enc,
        ours_mutated_enc,
        prost_enc: median_ns(samples, iters, || {
            let _ = prost_msg.encode_to_vec();
        }),
        v4_enc: median_ns(samples, iters, || {
            let _ = V4Serialize::serialize(&v4_msg).unwrap();
        }),
        buffa_enc: median_ns(samples, iters, || {
            let _ = BuffaMessage::encode_to_vec(&buffa_msg);
        }),
        ours_dec,
        prost_dec: median_ns(samples, iters, || {
            let _ = prost_tat::TestAllTypesProto3::decode(bytes.as_slice()).unwrap();
        }),
        v4_dec: median_ns(samples, iters, || {
            let _ = v4_tat::TestAllTypesProto3::parse(&bytes).unwrap();
        }),
        buffa_dec: median_ns(samples, iters, || {
            let _ = BuffaTat::decode_from_slice(&bytes).unwrap();
        }),
        buffa_view: Some(median_ns(samples, iters, || {
            let _ = BuffaTatView::decode_view(&bytes).unwrap();
        })),
        ours_def,
        iters,
        samples,
    }
}

fn run_person(iters: u32) -> Case {
    let msg = person_ours();
    let bytes = pbrs::Serialize::serialize(&msg).expect("person encode");
    let prost_msg = ProstPerson {
        id: msg.id(),
        name: msg.name().to_str().unwrap_or("").to_string(),
        email: msg
            .email_opt()
            .map(|s| s.to_str().unwrap_or("").to_string()),
        tags: msg
            .tags()
            .iter()
            .map(|s| s.as_view().to_str().unwrap_or("").to_string())
            .collect(),
        scores: msg
            .scores()
            .iter()
            .map(|(k, v)| (k.as_view().to_str().unwrap_or("").to_string(), v))
            .collect(),
        address: Some(ProstAddress {
            city: msg.address().city().to_str().unwrap_or("").to_string(),
        }),
    };
    let mut v4_msg = v4_person::Person::new();
    v4_msg.set_id(msg.id());
    v4_msg.set_name(msg.name().to_str().unwrap_or(""));
    v4_msg.set_email(msg.email().to_str().unwrap_or(""));
    for t in msg.tags().iter() {
        v4_msg.tags_mut().push(t.as_view().to_str().unwrap_or(""));
    }
    for (k, v) in msg.scores().iter() {
        v4_msg
            .scores_mut()
            .insert(k.as_view().to_str().unwrap_or(""), v);
    }
    v4_msg
        .address_mut()
        .set_city(msg.address().city().to_str().unwrap_or(""));
    let buffa_msg = buffa_person::example::Person {
        id: msg.id(),
        name: msg.name().to_str().unwrap_or("").to_string(),
        email: msg
            .email_opt()
            .map(|s| s.to_str().unwrap_or("").to_string()),
        tags: msg
            .tags()
            .iter()
            .map(|s| s.as_view().to_str().unwrap_or("").to_string())
            .collect(),
        scores: msg
            .scores()
            .iter()
            .map(|(k, v)| (k.as_view().to_str().unwrap_or("").to_string(), v))
            .collect(),
        address: Some(buffa_person::example::Address {
            city: msg.address().city().to_str().unwrap_or("").to_string(),
            ..Default::default()
        })
        .into(),
        ..Default::default()
    };
    let ours_dec = median_ns(15, iters, || {
        let _ = Person::parse(&bytes).unwrap();
    });
    let ours_enc = median_ns(15, iters, || {
        let _ = pbrs::Serialize::serialize(&msg).unwrap();
    });
    let parse_and_enc = median_ns(15, iters, || {
        let fresh = Person::parse(&bytes).unwrap();
        let _ = pbrs::Serialize::serialize(&fresh).unwrap();
    });
    let ours_fresh_enc = Some((parse_and_enc - ours_dec).max(ours_enc));
    let mut mutated = msg.clone();
    let ours_mutated_enc = Some(median_ns(15, iters, || {
        mutated.set_id(42);
        let _ = pbrs::Serialize::serialize(&mutated).unwrap();
    }));
    Case {
        name: "person",
        layout: "handwritten",
        payload: bytes.len(),
        ours_enc,
        ours_fresh_enc,
        ours_mutated_enc,
        prost_enc: median_ns(15, iters, || {
            let _ = prost_msg.encode_to_vec();
        }),
        v4_enc: median_ns(15, iters, || {
            let _ = V4Serialize::serialize(&v4_msg).unwrap();
        }),
        buffa_enc: median_ns(15, iters, || {
            let _ = BuffaMessage::encode_to_vec(&buffa_msg);
        }),
        ours_dec,
        prost_dec: median_ns(15, iters, || {
            let _ = ProstPerson::decode(bytes.as_slice()).unwrap();
        }),
        v4_dec: median_ns(15, iters, || {
            let _ = v4_person::Person::parse(&bytes).unwrap();
        }),
        buffa_dec: median_ns(15, iters, || {
            let _ = buffa_person::example::Person::decode_from_slice(&bytes).unwrap();
        }),
        buffa_view: Some(median_ns(15, iters, || {
            let _ = buffa_person::example::PersonView::decode_view(&bytes).unwrap();
        })),
        ours_def: None,
        iters,
        samples: 15,
    }
}

fn person_generated_ours() -> person_generated::Person {
    let mut addr = person_generated::Address::new();
    addr.set_city("nyc");
    let mut p = person_generated::Person::new();
    p.set_id(7);
    p.set_name("ada lovelace");
    p.set_email("ada@example.com");
    p.tags_mut().push("math");
    p.tags_mut().push("eng");
    p.scores_mut().insert("notes", 12);
    p.set_address(addr);
    p
}

/// Benchmark case using compiler-generated `Person` (from `proto/person.proto`).
///
/// This isolates and measures the exact difference between compiler-generated
/// schema layout (`Repeated<LazyStr>`, `Map<LazyStr, i32>`) and the handwritten
/// `pbrs::testdata::Person` with inline small repeats (`InlineVec`).
fn run_person_generated(iters: u32) -> Case {
    let msg = person_generated_ours();
    let bytes = pbrs::Serialize::serialize(&msg).expect("person_generated encode");
    let prost_msg = ProstPerson {
        id: msg.id(),
        name: msg.name().to_str().unwrap_or("").to_string(),
        email: msg
            .email_opt()
            .map(|s| s.to_str().unwrap_or("").to_string()),
        tags: msg
            .tags()
            .iter()
            .map(|s| s.as_view().to_str().unwrap_or("").to_string())
            .collect(),
        scores: msg
            .scores()
            .iter()
            .map(|(k, v)| (k.as_view().to_str().unwrap_or("").to_string(), v))
            .collect(),
        address: Some(ProstAddress {
            city: msg.address().city().to_str().unwrap_or("").to_string(),
        }),
    };
    let mut v4_msg = v4_person::Person::new();
    v4_msg.set_id(msg.id());
    v4_msg.set_name(msg.name().to_str().unwrap_or(""));
    v4_msg.set_email(msg.email().to_str().unwrap_or(""));
    for t in msg.tags().iter() {
        v4_msg.tags_mut().push(t.as_view().to_str().unwrap_or(""));
    }
    for (k, v) in msg.scores().iter() {
        v4_msg
            .scores_mut()
            .insert(k.as_view().to_str().unwrap_or(""), v);
    }
    v4_msg
        .address_mut()
        .set_city(msg.address().city().to_str().unwrap_or(""));
    let buffa_msg = buffa_person::example::Person {
        id: msg.id(),
        name: msg.name().to_str().unwrap_or("").to_string(),
        email: msg
            .email_opt()
            .map(|s| s.to_str().unwrap_or("").to_string()),
        tags: msg
            .tags()
            .iter()
            .map(|s| s.as_view().to_str().unwrap_or("").to_string())
            .collect(),
        scores: msg
            .scores()
            .iter()
            .map(|(k, v)| (k.as_view().to_str().unwrap_or("").to_string(), v))
            .collect(),
        address: Some(buffa_person::example::Address {
            city: msg.address().city().to_str().unwrap_or("").to_string(),
            ..Default::default()
        })
        .into(),
        ..Default::default()
    };
    let ours_dec = median_ns(15, iters, || {
        let _ = person_generated::Person::parse(&bytes).unwrap();
    });
    let ours_enc = median_ns(15, iters, || {
        let _ = pbrs::Serialize::serialize(&msg).unwrap();
    });
    let parse_and_enc = median_ns(15, iters, || {
        let fresh = person_generated::Person::parse(&bytes).unwrap();
        let _ = pbrs::Serialize::serialize(&fresh).unwrap();
    });
    let ours_fresh_enc = Some((parse_and_enc - ours_dec).max(ours_enc));
    let mut mutated = msg.clone();
    let ours_mutated_enc = Some(median_ns(15, iters, || {
        mutated.set_id(42);
        let _ = pbrs::Serialize::serialize(&mutated).unwrap();
    }));
    Case {
        name: "person_generated",
        layout: "generated",
        payload: bytes.len(),
        ours_enc,
        ours_fresh_enc,
        ours_mutated_enc,
        prost_enc: median_ns(15, iters, || {
            let _ = prost_msg.encode_to_vec();
        }),
        v4_enc: median_ns(15, iters, || {
            let _ = V4Serialize::serialize(&v4_msg).unwrap();
        }),
        buffa_enc: median_ns(15, iters, || {
            let _ = BuffaMessage::encode_to_vec(&buffa_msg);
        }),
        ours_dec,
        prost_dec: median_ns(15, iters, || {
            let _ = ProstPerson::decode(bytes.as_slice()).unwrap();
        }),
        v4_dec: median_ns(15, iters, || {
            let _ = v4_person::Person::parse(&bytes).unwrap();
        }),
        buffa_dec: median_ns(15, iters, || {
            let _ = buffa_person::example::Person::decode_from_slice(&bytes).unwrap();
        }),
        buffa_view: Some(median_ns(15, iters, || {
            let _ = buffa_person::example::PersonView::decode_view(&bytes).unwrap();
        })),
        ours_def: None,
        iters,
        samples: 15,
    }
}

fn gated(name: &str) -> bool {
    matches!(
        name,
        "empty"
            | "person"
            | "tat_populated"
            | "packed_256"
            | "map_64"
            | "nested_8"
            | "strings"
            | "unpacked_256"
            | "packed_fixed_256"
            | "packed_fixed64_256"
            | "packed_float_256"
            | "repeated_nested_8"
    )
}

fn view_gated(name: &str) -> bool {
    // tat_populated / person vs buffa view sit in a ~3% band. Do not fail
    // the process on them. Packed-fixed view is also ungated (no owned Vec).
    matches!(
        name,
        "empty" | "packed_256" | "map_64" | "nested_8" | "strings"
    )
}

fn main() {
    let iters = 40_000u32;
    let cases = [
        run_tat("empty", TestAllTypesProto3::new(), iters),
        run_person(iters),
        run_person_generated(iters),
        run_tat("tat_populated", tat_populated(), iters),
        run_tat("packed_256", tat_packed(), iters),
        run_tat("map_64", tat_map(), iters),
        run_tat("nested_8", tat_nested(), iters),
        run_tat("strings", tat_strings(), iters),
        run_tat("unpacked_256", tat_unpacked(), iters),
        run_tat("packed_fixed_256", tat_packed_fixed(), iters),
        run_tat("bytes", tat_bytes(), iters),
        run_tat("scalars", tat_scalars(), iters),
        run_tat("packed_fixed64_256", tat_packed_fixed64(), iters),
        run_tat("packed_float_256", tat_packed_float(), iters),
        run_tat("unpacked_fixed_256", tat_unpacked_fixed(), iters),
        run_tat("oneof", tat_oneof(), iters),
        run_tat("repeated_nested_8", tat_repeated_nested(), iters),
        run_tat("bytes_1mb", tat_bytes_n(1_000_000), iters),
        run_tat("bytes_5mb", tat_bytes_n(5_000_000), iters),
        run_tat("packed_fixed_1mb", tat_packed_fixed_n(250_000), iters),
        run_tat("packed_fixed_5mb", tat_packed_fixed_n(1_250_000), iters),
    ];

    println!("{{");
    println!(
        "  \"sizeof_tat\": {},",
        std::mem::size_of::<TestAllTypesProto3>()
    );
    println!("  \"iters\": {iters},");
    println!("  \"cases\": [");
    for (i, c) in cases.iter().enumerate() {
        let comma = if i + 1 == cases.len() { "" } else { "," };
        println!("    {{");
        println!("      \"name\": \"{}\",", c.name);
        println!("      \"layout\": \"{}\",", c.layout);
        println!("      \"payload_bytes\": {},", c.payload);
        println!("      \"iters\": {},", c.iters);
        println!("      \"samples\": {},", c.samples);
        println!("      \"ours_encode_ns\": {:.3},", c.ours_enc);
        println!("      \"ours_cached_encode_ns\": {:.3},", c.ours_enc);
        match c.ours_fresh_enc {
            Some(v) => println!("      \"ours_fresh_encode_ns\": {v:.3},"),
            None => println!("      \"ours_fresh_encode_ns\": null,"),
        }
        match c.ours_mutated_enc {
            Some(v) => println!("      \"ours_mutated_encode_ns\": {v:.3},"),
            None => println!("      \"ours_mutated_encode_ns\": null,"),
        }
        println!("      \"prost_encode_ns\": {:.3},", c.prost_enc);
        println!("      \"v4_encode_ns\": {:.3},", c.v4_enc);
        println!("      \"buffa_encode_ns\": {:.3},", c.buffa_enc);
        println!("      \"ours_decode_ns\": {:.3},", c.ours_dec);
        println!("      \"ours_decode_owned_ns\": {:.3},", c.ours_dec);
        println!("      \"prost_decode_ns\": {:.3},", c.prost_dec);
        println!("      \"prost_decode_owned_ns\": {:.3},", c.prost_dec);
        println!("      \"v4_decode_ns\": {:.3},", c.v4_dec);
        println!("      \"v4_decode_owned_ns\": {:.3},", c.v4_dec);
        println!("      \"buffa_decode_ns\": {:.3},", c.buffa_dec);
        println!("      \"buffa_decode_owned_ns\": {:.3},", c.buffa_dec);
        match c.buffa_view {
            Some(v) => {
                println!("      \"buffa_view_decode_ns\": {v:.3},");
                println!("      \"buffa_decode_view_ns\": {v:.3},");
            }
            None => {
                println!("      \"buffa_view_decode_ns\": null,");
                println!("      \"buffa_decode_view_ns\": null,");
            }
        }
        match c.ours_def {
            Some(v) => println!("      \"ours_default_ns\": {v:.3},"),
            None => println!("      \"ours_default_ns\": null,"),
        }
        println!(
            "      \"ours_faster_v4_encode\": {},",
            c.ours_enc < c.v4_enc
        );
        println!(
            "      \"ours_faster_v4_decode\": {},",
            c.ours_dec < c.v4_dec
        );
        println!(
            "      \"ours_faster_buffa_encode\": {},",
            c.ours_enc < c.buffa_enc
        );
        println!(
            "      \"ours_faster_buffa_decode\": {},",
            c.ours_dec < c.buffa_dec
        );
        println!(
            "      \"ours_faster_prost_encode\": {},",
            c.ours_enc < c.prost_enc
        );
        println!(
            "      \"ours_faster_prost_decode\": {},",
            c.ours_dec < c.prost_dec
        );
        match c.buffa_view {
            Some(v) => println!(
                "      \"ours_faster_buffa_view_decode\": {},",
                c.ours_dec < v
            ),
            None => println!("      \"ours_faster_buffa_view_decode\": null,"),
        }
        println!("      \"gated\": {}", gated(c.name));
        println!("    }}{comma}");
    }
    println!("  ]");
    println!("}}");

    let mut failed = false;
    for c in &cases {
        if !gated(c.name) {
            continue;
        }
        if c.ours_enc >= c.prost_enc || c.ours_dec >= c.prost_dec {
            eprintln!("perf gate failed: {} vs prost", c.name);
            failed = true;
        }
        if c.ours_enc >= c.v4_enc || c.ours_dec >= c.v4_dec {
            eprintln!("perf gate failed: {} vs v4", c.name);
            failed = true;
        }
        if c.ours_enc >= c.buffa_enc || c.ours_dec >= c.buffa_dec {
            eprintln!("perf gate failed: {} vs buffa owned", c.name);
            failed = true;
        }
        if view_gated(c.name) {
            if let Some(v) = c.buffa_view {
                if c.ours_dec >= v {
                    eprintln!("perf gate failed: {} vs buffa view", c.name);
                    failed = true;
                }
            }
        }
    }
    if failed {
        std::process::exit(1);
    }
}
