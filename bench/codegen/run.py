...
def render_consumer(messages: int, marker: int) -> str:
    lines = [
...
def manifest(name: str) -> str:
    return (
        f'[package]\nname = "{name}"\nversion = "0.0.0"\nedition = "2024"\n'
        'publish = false\n\n[workspace]\n\n[dependencies]\n'
        f"pbrs = {{ path = {json.dumps(str(ROOT))} }}\n\n"
        '[profile.release]\nopt-level = 3\nlto = "thin"\ncodegen-units = 1\n'
    )
...
def snapshot_generated(out: Path) -> dict[str, tuple[int, str, int]]:
    if not (out / "mod.rs").is_file():
        raise BenchmarkError(f"generation omitted {out / 'mod.rs'}")
...
def phase(
    ...
    try:
        measurement = timed_command(command, cwd, env, stem, run_dir, timeout, sample_ms)
...
def provenance(run_dir: Path, jobs: int, sample_ms: int) -> dict:
    ...
    protoc = tool_version(os.environ.get("PROTOC", "protoc"), ["--version"], "protoc", run_dir)
    return {
        "repository": {
            "head": head,
            "tracked_changes": dirty,
            "source_sha256": source_hashes(),
            "source_scope": "src/**/*.rs, build.rs, proto/person.proto, vendored conformance FDS, Cargo.toml/Cargo.lock, harness, generator and wrapper",
        },
        "host": {
...
def run_cases(report: dict, run_dir: Path, cases: list[str], seed: int, jobs: int, timeout: int, sample_ms: int) -> None:
    details = provenance(run_dir, jobs, sample_ms)
    ...
    for case in cases:
        ...
        release_env = {**check_env, "CARGO_INCREMENTAL": "0"}
        ...
        write_text(main_rs, render_consumer(CORPORA[case][0], 1))
        ...
        release = phase(
            ...
            ROOT, release_env, run_dir, timeout, sample_ms, logs / "build-release",
        )
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        help="fail fast: no equivalent pinned reference generator is wired yet",
    )
    ...
    report = {
        ...
        "qualification": {
            "qualified": False,
            "reasons": ["no_equivalent_reference_peer", "single_run_diagnostic"],
        },
        "errors": [],
    }
    ...
    if args.require_qualified:
        ...
        write_report(report, run_dir)
        print(f"UNQUALIFIED: missing reference peer; {run_dir / 'summary.json'}", file=sys.stderr)
        return 2
    try:
        run_cases(report, run_dir, cases, args.seed, args.jobs, args.timeout_seconds, args.rss_sample_ms)
    except (BenchmarkError, OSError, subprocess.CalledProcessError, ValueError) as exc:
        report["status"] = "error"
