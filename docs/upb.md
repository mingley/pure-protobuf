...
| JSON / text (specialized in upb/C++) | Field-wise JSON and text for Person-shaped proto3, the extra proto3 scalars (bool / int64 / uint32 / uint64 / sint / fixed / sfixed / float / double / bytes / open enums, including repeated and scalar maps), real oneofs of that set (`OneofHole`), and `google.protobuf.Timestamp` / `Duration` / `Empty` / proto3 wrappers / `FieldMask` (official proto3 JSON: strings / `{}` / wrapped value / comma-separated paths). Map-of-enum is skipped (map-entry descriptors lack enum names at codegen). TAT and the other WKT (Struct, Value, ListValue, Any) still serialize, then `DynamicMessage` | Correct (conformance). Not a JSON/text microbench winner. TAT is not closed. |
