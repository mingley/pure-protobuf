...
| JSON / text (specialized in upb/C++) | Field-wise JSON and text for Person-shaped proto3, the extra proto3 scalars (bool / int64 / uint32 / uint64 / sint / fixed / sfixed / float / double / bytes / open enums, including repeated and scalar maps), and real oneofs of that set (`OneofHole`). Map-of-enum is skipped (map-entry descriptors lack enum names at codegen). TAT / WKT still serialize, then `DynamicMessage` | Correct (conformance). Not a JSON/text microbench winner. TAT is not closed. |
