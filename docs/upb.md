...
| JSON / text (specialized in upb/C++) | Field-wise JSON for Person-shaped proto3 (`Person`, `hello`). Other generated JSON and all text still serialize, then `DynamicMessage` | Correct (conformance). Not a JSON microbench winner. TAT / WKT JSON still go through `DynamicMessage`. |
