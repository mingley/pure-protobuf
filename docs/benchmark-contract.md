...
The QPS runner likewise defaults to the root Cargo target with at most two
build jobs and reruns the incremental native-worker build on each ordinary
invocation rather than trusting an older executable. Explicit `--skip-build`
runs retain the binary digest but leave its source SHA unverified.
For the v4 codec, CI verifies and uses checked Rust output from the pinned
protobuf v35.1 generator; a changed `codec_cases.proto` or generated byte
sequence fails closed. Normal local generation requires genuine pinned
`protoc 35.1`, not a newer compiler with its generated version rewritten to
match the older runtime.
